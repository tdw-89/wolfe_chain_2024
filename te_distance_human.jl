...
only_write_bed_files = true
repeat_types = ["LINE", "SINE", "DNA", "Retroposon", "LTR", "TE"]
repeat_type = repeat_types[end]
...
if only_specific_type
    if repeat_type == "TE"
        ...
        if repeat_type in te_df.Type
            filter!(row -> row.Type == repeat_type, te_df)
        elseif repeat_type in te_df.Family
            filter!(row -> row.Family == repeat_type, te_df)
        end
    end
end
...
if repeat_type == "TE" && !only_h3k9me3
    te_bed_file = "../../dicty_data/te_density/te_coordinates_human.bed"
    open(te_bed_file, "w") do file
        for (scaffold_name, scaffold) in human_ref.scaffolds
...
# Write the non-TE CDS gene coordinates to a BED file for use in downstream analyses.
if repeat_type == "TE" && !only_h3k9me3
    cds_bed_file = "../../dicty_data/te_density/cds_coordinates_human.bed"
...
file_suffix = only_specific_type ? "_$repeat_type" : ""
